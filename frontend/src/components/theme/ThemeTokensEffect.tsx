...
export function ThemeTokensEffect({ theme }: ThemeTokensEffectProps) {
  ...
  React.useEffect(() => {
    ...
    if (tokens.popoverForeground)
      root.style.setProperty('--popover-foreground', normalizeThemeToken(tokens.popoverForeground));
    if (tokens.primary) root.style.setProperty('--primary', normalizeThemeToken(tokens.primary));
    if (tokens.primaryForeground)
      root.style.setProperty('--primary-foreground', normalizeThemeToken(tokens.primaryForeground));
    if (tokens.secondary) root.style.setProperty('--secondary', normalizeThemeToken(tokens.secondary));
    if (tokens.secondaryForeground)
      root.style.setProperty('--secondary-foreground', normalizeThemeToken(tokens.secondaryForeground));
    if (tokens.muted) root.style.setProperty('--muted', normalizeThemeToken(tokens.muted));
    if (tokens.mutedForeground)
      root.style.setProperty('--muted-foreground', normalizeThemeToken(tokens.mutedForeground));
    if (tokens.accent) root.style.setProperty('--accent', normalizeThemeToken(tokens.accent));
    if (tokens.accentForeground)
      root.style.setProperty('--accent-foreground', normalizeThemeToken(tokens.accentForeground));
    if (tokens.destructive) root.style.setProperty('--destructive', normalizeThemeToken(tokens.destructive));
    if (tokens.destructiveForeground)
      root.style.setProperty('--destructive-foreground', normalizeThemeToken(tokens.destructiveForeground));
