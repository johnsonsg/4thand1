...
export function HeroSection({ rendering }: HeroSectionProps) {
 ...
 const defaultBackground = {
	...
	const bgValue = fields.backgroundImage?.value as
		| { src?: string; url?: string; alt?: string }
		| undefined

	const background =
		bgValue?.src || bgValue?.url
		? { src: bgValue.src ?? bgValue.url!, alt: bgValue.alt ?? 'Hero background' }
		: defaultBackground

	const season = fields.season?.value ?? '2025-2026 Season'
	const headline = fields.headline?.value ?? 'Friday Night Lights'
	const heroDescription =
		fields.heroDescription?.value ??
